...
import theanoimport theano.tensor as Tfrom theano.tensor.nnet import convfrom theano.tensor.signal import downsampleimport numpy as npfrom sklearn.datasets import fetch_mldatafrom sklearn.cross_validation import train_test_split
def relu(x): return T.maximum(0, x)def sigmoid(x): return T.nnet.sigmoid(x)def tanh(x): return T.tanh(x)
def dropout(rng, x, train, p=0.5):	masked_x = None if p > 0.0 and p < 1.0:		seed = rng.randint(2 ** 30)		srng = theano.tensor.shared_randomstreams.RandomStreams(seed)		mask = srng.binomial( n=1, p=1.0-p, size=x.shape, dtype=theano.config.floatX		) # mask *= 1.0 / (1.0 - p)		masked_x = x * mask else:		masked_x = x return T.switch(T.neq(train, 0), masked_x, x*(1.0-p))
def get_corrupted_input(rng, x, train, corruption_level=0.3):	masked_x = None if corruption_level > 0.0 and corruption_level < 1.0:		seed = rng.randint(2 ** 30)		srng = theano.tensor.shared_randomstreams.RandomStreams(seed)		mask = srng.binomial( n=1, p=1.0-corruption_level, size=x.shape, dtype=theano.config.floatX		)		masked_x = x * mask return T.switch(T.neq(train, 0), masked_x, x)
