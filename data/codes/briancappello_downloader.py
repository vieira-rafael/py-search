...
import requestsimport grequestsfrom gevent import monkeymonkey.patch_all()
from pytradelib.utils import batchfrom pytradelib.logger import logger

class Downloader(object): def __init__(self, batch_size=25, sleep=None): self._batch_size = batch_size self._sleep = sleep
 @property def batch_size(self): return self._batch_size
 @batch_size.setter def batch_size(self, batch_size): self._batch_size = batch_size
 @property def sleep(self): return self._sleep
 @sleep.setter def sleep(self, sleep): self._sleep = sleep
 def download(self, urls): if isinstance(urls, str): return self._download(urls) return self._bulk_download(urls)
 ...
 def __bulk_download(self, urls, errors=None):        errors = errors or []
 def log_response(response, **kwargs):            logger.info('Download completed: ' + response.url)
 ...
 return grequests.map((grequests.get(url, callback=log_response) for url in urls), exception_handler=exception_handler)