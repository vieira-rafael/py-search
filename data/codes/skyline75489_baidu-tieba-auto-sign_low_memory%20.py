# -*- coding: utf-8 -*-import urllibimport urllib2import cookielibimport reimport hashlibimport jsonimport threadingfrom Queue import Queue  import platformimport os
...
def worker(temp, BDUSS): while not q.empty():        tieba = q.get() try:            _sign_tieba(tieba, BDUSS) finally:            q.task_done()  def sign(my_cookie, BDUSS):    _setup_cookie(my_cookie)    _like_tieba_list = _fetch_like_tieba_list() if len(_like_tieba_list) == 0: print u"CookieBDUSS" if system_env else "CookieBDUSS" return    thread_list = []
 map(q.put,_like_tieba_list)    threads = [threading.Thread(target=worker, args=(1, BDUSS)) for i in xrange(10)] map(lambda x:x.start(),threads)    q.join()
def main():    my_cookie = "Cookie" BDUSS = "BDUSS"    sign(my_cookie, BDUSS)
if __name__ == "__main__":    system_env = True if platform.system()=='Windows' else False    q = Queue()    main()    os.system("date /T >> tieba_log.log") if system_env else os.system("date >> tieba_log.log")