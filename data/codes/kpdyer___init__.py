import stringimport mathimport binasciiimport functools
import gmpy as gmpy
from Crypto.Cipher import AESfrom Crypto.Util import Counter
_gmpy_mpz_type = type(gmpy.mpz(0))_gmpy_mpf_type = type(gmpy.mpf(0))

def new(K, radix): return FFXEncrypter(K, radix)

class Bottom(Exception): pass

class UnknownTypeException(Exception): pass

class InvalidRadixException(Exception): pass

def long_to_bytes(N, blocksize=1): """Given an input integer ``N``, ``long_to_bytes`` returns the representation of ``N`` in bytes.    If ``blocksize`` is greater than ``1`` then the output string will be right justified and then padded with zero-bytes,    such that the return values length is a multiple of ``blocksize``. """  if type(N) == FFXInteger: return N.to_bytes()     bytestring = gmpy.digits(N, 16)    bytestring = '0' + bytestring if (len(bytestring) % 2) != 0 else bytestring    bytestring = binascii.unhexlify(bytestring)
 if blocksize>0 and (len(bytestring) % blocksize) != 0:        bytestring = '\x00' * \            (blocksize - (len(bytestring) % blocksize)) + bytestring
 return bytestring
...
class FFXEncrypter(object):
 def __init__(self, K, radix): if radix not in range(2, 37): raise InvalidRadixException()
 self._radix = gmpy.mpz(radix) self._chars = string.digits + string.ascii_lowercase self._chars = self._chars[:radix]        _chars = [] for c in self._chars:            _chars.append(c) self._chars = _chars
 self._K = K self._ecb = AES.new(K, AES.MODE_ECB) self._P = {}
 def isEven(self, n): return not ((n & 0x1) == 1)
 def add(self, X, Y):        retval = (X + Y) % (X._radix ** (X._blocksize)) return FFXInteger(retval, radix=self._radix, blocksize=X._blocksize)
 def sub(self, X, Y):        retval = (X - Y) % (X._radix ** (X._blocksize)) return FFXInteger(retval, radix=self._radix, blocksize=X._blocksize)
 #def rnds(self, n): #    return 10
 def split(self, n): """TODO""" return int(math.floor((n * 1.0) / 2))
 def F(self, n, T, i, B): if T == 0:            t = 0 else:            t = len(T)
        beta = math.ceil(n / 2.0)        b = int(math.ceil(math.ceil(beta * math.log(self._radix, 2)) / 8.0))        d = 4 * int(math.ceil(b / 4.0))
 if self.isEven(i):            m = int(math.floor(n / 2.0)) else:            m = int(math.ceil(n / 2.0))
 if not self._P.get(n): P = '\x01' # vers P += '\x02' # method P += '\x01' # addition P += long_to_bytes(self._radix, 3) P += '\x0a' # always ten P += long_to_bytes(self.split(n)%256, 1) P += long_to_bytes(n, 4) P += long_to_bytes(t, 4) self._P[n] = P
 if T == 0: Q = '' else: Q = str(T)  Q += '\x00' * (((-1 * t) - b - 1) % 16) Q += long_to_bytes(i, blocksize=1)  _B_as_bytes = long_to_bytes(B) Q += '\x00' * (b - len(_B_as_bytes)) Q += _B_as_bytes[-b:]
        _cbc = AES.new(self._K, AES.MODE_CBC, '\x00' * 16)
 assert len(self._P[n]) % 16 == 0 assert len(Q) % 16 == 0
 Y = _cbc.encrypt(self._P[n] + Q)[-16:]
        i = 1 TMP = Y while len(TMP) < (d + 4):            left = FFXInteger(bytes_to_long(Y), radix=self._radix, blocksize=32)            right = FFXInteger(str(i), radix=10, blocksize=16) X = self.add(left, right) TMP += self._ecb.encrypt(X.to_bytes(16))            i += 1
        y = bytes_to_long(TMP[:(d + 4)])        z = y % (self._radix ** m)
 return z
 def encrypt(self, T, X):        retval = ''
        n = len(X)        l = self.split(n)        r = 10 #self.rnds(n) A = X[:l] B = X[l:] for i in range(r): C = self.add(A, self.F(n, T, i, B)) A = B B = C
        retval = FFXInteger(str(A) + str(B), radix=self._radix)  return retval
 def decrypt(self, T, Y):        retval = ''         n = len(Y)        l = self.split(n)        r = 10 #self.rnds(n)
 A = Y[:l] B = Y[l:] for i in range(r - 1, -1, -1): C = B B = A A = self.sub(C, self.F(n, T, i, B))         retval = FFXInteger(str(A) + str(B), radix=self._radix)
 return retval