...
import urllib.request, urllib.error, urllib.parseimport sysimport reimport http.cookiejarimport base64from urllib.parse import urlparsefrom xml.dom.minidom import parse, parseStringimport xml.dom.minidomimport array, platform, struct, fcntl, socketimport hashlibimport math
ImportCookie = FalseVerbose = TrueProtocol = NonecookieJar = None
class colors: BLACK = '\033[00;30m' DGRAY = '\033[01;30m' BROWN = '\033[00;33m' BLUE = '\033[00;94m' GREEN = '\033[00;92m' DGREEN = '\033[01;92m' YELLOW = '\033[00;93m' DYELLOW = '\033[00;93m' ORANGE = '\033[01;31m' DRED = '\033[01;91m' RED = '\033[00;91m' VIOLET = '\033[00;35m' ENDC = '\033[0m' def disable(self): BLACK = '' DGRAY = '' BROWN = '' BLUE = '' GREEN = '' DGREEN = '' YELLOW = '' DYELLOW = '' ORANGE = '' DRED = '' RED = '' VIOLET = '' ENDC = ''

def toNowhere(var, var2=None): pass
def cookieOut(val): pass
def formatCookie(cook):	ret = ' '	attributes = cook[1].split(';')	content = '' for a in attributes: if content != '':			content += ';'
 if a.strip().lower() == 'httponly':			content += colors.DRED + a + colors.ENDC elif a.strip().lower() == 'secure':			content += colors.DRED + a + colors.ENDC else:			cookieOut(a)			content += a	ret += colors.DYELLOW + cook[0] + colors.ENDC + ': ' + content return ret
def defaultPrintHeaders(headers, hfilter=None): if hfilter != None:		hfilter = hfilter.lower() if type(headers) == type({}):		h1 = [] for i in headers.keys(): if i.lower() == "cookie" and type(headers[i]) == type({}): for c in headers[i].keys():					h1.append(("Cookie",c + "=" + headers[i][c])) else:				h1.append((i,headers[i]))		headers = h1  for i in headers: if i[0].lower() == hfilter or hfilter == None: if i[0].lower() == 'set-cookie' or i[0].lower() == 'cookie':				printOut(formatCookie(i)) else:				printOut(' ' + colors.DGRAY + i[0] + colors.ENDC + ': ' + str(i[1]) )
printHeaders = defaultPrintHeaders
def defaultPrintRequestOut(method, additional): if Verbose:		s = '' if method.strip().upper() == 'GET':			s += colors.VIOLET + method + colors.ENDC elif method.strip().upper() == 'POST':			s += colors.BLUE + method + colors.ENDC else:			s += colors.BROWN + method + colors.ENDC		s += ' ' + additional		sys.stdout.write(str(s) + "\n")printRequest = defaultPrintRequestOut
def printData(data): if Verbose: if type(data) == type(b''):			data = data.decode()		sys.stdout.write(data)
def defaultPrintAnswerOut(code,additional): if Verbose:		ret = ' ' if code == 200:			ret += colors.DGREEN + '200 ' + colors.ENDC elif code == 302:			ret += colors.ORANGE + '302 ' + colors.ENDC else:			ret += colors.DRED + str(code) + colors.ENDC + ' '		ret += str(additional)		ret += '\n'		sys.stdout.write(ret)
printAnswer = defaultPrintAnswerOut
def defaultPrintOut(s): if Verbose:		sys.stdout.write(str(s) + "\n")		sys.stdout.flush()
printOut = defaultPrintOut
def defaultWriteOut(s): if Verbose:		sys.stdout.write(str(s))
writeOut = defaultWriteOut
def defaultDebug(s): if Verbose:		sys.stdout.write('Debug: ' + str(s) + '\n')
printDebug = defaultDebug
def outputControl(requests = True, headers = False , response = True, misc = False, debug = False): if not headers:			printHeaders = toNowhere
 if not requests:		printRequest = toNowhere else:		printRequest = defaultPrintRequestOut  if not response:		printAnswer = toNowhere else:		printAnswer = defaultPrintAnswerOut  if not misc:		printOut = toNowhere else:		printOut = defaultPrintOut
 if not debug:		printDebug = toNowhere else:		printDebug = defaultDebug
 def headersValue(headerList):	res = '[' for i in headerList:		res += str(i) + ' '	res = res[:-1] + ']' return res
def b64decode(s):	finals = s
 print((len(s)))	modres = len(s) % 4 if len(re.findall('=',s)) != 0: return base64.b64decode(s) elif modres == 0: return base64.b64decode(s) elif modres == 1:		finals = s + 'w=='  elif modres == 2:		finals = s + '==' else:		finals = s + '='
 print(finals)
 return base64.b64decode(finals)
def all_interfaces(): SIOCGIFCONF = 0x8912 MAXBYTES = 8096
	s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)	names = array.array('B', b'\0' * MAXBYTES) 	arch = platform.architecture()[0]	var1 = -1	var2 = -1 if arch == '32bit':	  var1 = 32	  var2 = 32 elif arch == '64bit':	  var1 = 16	  var2 = 40
	outbytes = struct.unpack('iL', fcntl.ioctl(s.fileno(), SIOCGIFCONF, struct.pack('iL', MAXBYTES, names.buffer_info()[0])))[0]
	namestr = names.tobytes()	dic = {} for i in range(0, outbytes, var2):		dic[namestr[i:i+var1].decode().split('\0', 1)[0]] = socket.inet_ntoa(namestr[i+20:i+24]) return dic
def birthdayProblem(space,sampleNumber): return 1 - math.exp(-((sampleNumber**2)/(2*space)))
def hexify(val,sep = " ", lblocksize = 0, linesize = 0): if type(val) == type(""):		val = val.encode() if linesize != 0 and lblocksize == 0:		linesize = 0 	i = 0	c = 0	fi = "" while i < len(val): if i != 0: if lblocksize != 0 and i % lblocksize == 0:				fi += " " if linesize != 0 and i % (lblocksize * linesize) == 0:				fi += "\n" 		fi += "{0:02x}".format(val[i]) + sep		i += 1
 return fi
def decify(val,printable = False): if type(val) == type(""):		val = val.encode()	i = 0	c = 0	fi = ""	sep = " " if printable:		sep = "\n" while i < len(val): if i != 0 and i % 16 == 0:			fi += sep elif i != 0 and i % 8 == 0:			fi += " "		fi += "{0:03d} ".format(val[i])		i += 1
 return fi
def intToBits(integer): return str(integer) if integer<=1 else intToBits(integer>>1) + str(integer&1)
def stringToBits(s):	res = '' for c in s:		res += intToBits(ord(c)) return res
def charaterSet(string=None,filename=None):	charSpace=set() if filename != None:		opendFile = open(filename, "r") for line in opendFile:			charSpace = charSpace | set(line.strip())		opendFile.close() elif string != None:		charSpace = set(string)	 return list(charSpace)
def decodeASP(text): """ base64 decode function for (ASP).NET """	isbytes = True if not isinstance(text, bytes):		text = text.encode()		isbytes = False
 try:		count = int(text[-1:])		text = text[:-1] for i in range(count):			text += b'=' except ValueError: pass
